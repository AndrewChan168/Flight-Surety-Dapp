...
*/
    ...
    function generateInsuranceKey(address _buyer, bytes32 _flightKey) public pure returns(bytes32){
        return keccak256(abi.encodePacked(_buyer, _flightKey));
    }
...
*/
     ...
     */
    function addInsurance(address _buyer, bytes32 _flightKey, address _airline, uint256 _fee) requireLessThanMax(_fee) 
    public{
        bytes32 insuranceKey = generateInsuranceKey(_buyer, _flightKey);
    ...
    */
    ...
    function queryInsuranceInfo(bytes32 _insuranceKey) public view returns(address buyer, bytes32 flightKey, address airline, uint256 fee, uint256 credits, InsuranceStatus insuranceStatus) {
    ...
    function queryInsuranceFee(bytes32 _insuranceKey) public view returns(uint256){
        require(isExist(_insuranceKey), "Insurance doesn't exist");
        return insurances[_insuranceKey].fee;
    }
}